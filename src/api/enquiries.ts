...
// GET ONE
export async function getEnquiryById(id: string): Promise<Enquiry> {
  const { data } = await axiosClient.get(`/enquiries/${id}`);
  return data.data;
}
...
// ADD FOLLOW-UP
export async function addFollowUp(id: string, payload: any) {
...
// MARK ADMITTED
export async function markEnquiryAdmitted(id: string, payload: any) {
  const { data } = await axiosClient.post(`/enquiries/${id}/admit`, payload);
  return data;
}

// MARK LOST
export async function markEnquiryLost(id: string, payload: any) {
  const { data } = await axiosClient.post(`/enquiries/${id}/lost`, payload);
  return data;
}
